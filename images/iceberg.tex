...
% Syntactic analysis
\node [below=45pt of code-03-tex.south west, anchor=north west, text width=7.6cm, align=left, xshift=-5pt] (code-07-tex) {%
  \small
  \sethlcolor{pink}%
  \begin{enumerate}
  \item A call to ``\texttt{\textbackslash iow\_new:N}'' with the argument ``\texttt{\textbackslash l\_example\_iow}''
  \item A call to ``\texttt{\textbackslash iow\_open:Nn}'' with arguments ``\texttt{\textbackslash l\_example\_iow}'' and ``\{ \texttt{example.idx} \}''
  \item A call to ``\texttt{\textbackslash cs\_new:Nn}'' with arguments ``\texttt{\textbackslash \_\_example\_foo:n}'' and ``\texttt{\{ \textbackslash cs\_new:Nn\,\ldots\,\}}''
  \item ``\texttt{\textbackslash cs\_generate\_variant:Nn}'' with arguments ``\texttt{\textbackslash \_\_example\_foo:n}'' and ``\texttt{\{ V, x \}}''
  \item ``\texttt{\textbackslash \_\_example\_foo:n}'' with ``\texttt{\{ Hello \}}''
  \item ``\texttt{\textbackslash example\_bar:nn}'' with \textcolor{red}{\hl{``\texttt{w}''}} and ``\texttt{\{ orld \}}''
  \end{enumerate}
};
\node [below=10pt of code-07-tex.south, anchor=north, text width=7.6cm, align=center, xshift=5pt] (code-07-label) {%
  \small
  (c) Syntactic analysis identifies six expl3 calls and 11~unrecognized tokens ``\texttt{\textbackslash begin\{document\}}'' and warns about missing braces around the \mbox{\texttt{n}-type} argument~\cite[Section~1.1]{latex2025interface3} ``\texttt{w}'' on line 13.
};
\end{tikzpicture}
\endgroup
\vspace{5pt}%